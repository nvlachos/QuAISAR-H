...
process_samples()	{
	...
	python "${shareScript}/removeShortContigs.py" -i "${OUTDATADIR}/${filename}/plasmidAssembly/scaffolds.fasta" -t 500 -s "normal_SPAdes"
	...
	if [[ -s "${OUTDATADIR}/${filename}/plasmidAssembly/${filename}_plasmid_scaffolds_trimmed.fasta" ]]; then
		...
		python3 "${shareScript}/fasta_headers.py" -i "${OUTDATADIR}/${filename}/plasmidAssembly/${filename}_plasmid_scaffolds_trimmed_original.fasta" -o "${OUTDATADIR}/${filename}/plasmidAssembly/${filename}_plasmid_scaffolds_trimmed.fasta"
	fi
