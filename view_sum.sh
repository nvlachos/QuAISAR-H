...
if [[ -z "${2}" ]]; then
	...
	sum_name=$(find ${processed}/${proj}/*.sum -maxdepth 1 -type f -printf '%d\0%p\n' | sort -rt '\0' -n ) #| head -n 1)
	...
	if [[ "${tool_status}" == "ALERT" ]]; then
		...
		#	failures=$(( failures + 1 ))
			...
			if [[ -s "${processed}/${1}/${sample_name}/16s/${1}_16s_blast_id.txt" ]]; then
				failure_flags="${failure_flags}-Genus_not_assigned_in_16s_output"
				failures=$(( failures + 1 ))
			else
				failure_flags="${failure_flags}-NO_16s_best_species"
				failures=$(( failures + 1 ))
			fi
		elif [[ "${tool}" == "16s_largest_hit" ]]; then
			if [[ -s "${processed}/${1}/${sample_name}/16s/${1}_16s_blast_id.txt" ]]; then
			...
			if [[ -d "${processed}/${1}/${sample_name}/plasmid/" ]]; then
