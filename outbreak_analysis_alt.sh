...
#
# Usage ./outbreak_analysis.sh path_to_list gapped/ungapped (analysis ran) identity (80/95/98/99/100) output_directory(will create a folder at this location with name of analysis_identifier) analysis_identifier(e.g. outbreak identifier) MATRIX|SNV|BOTH path_to_alt_DB clobberness[keep|clobber]
#
...
# Default script to keep previous data unless changed with input parameters
clobberness="keep"
...
# Shows a brief uasge/help section if -h option used as first argument
	...
	echo "Usage is ./outbreak_analysis.sh path_to_list_file gapped/ungapped 80/95/98/99/100 output_directory output_prefix path_to_alt_DB matrix|snv|both] clobberness[keep|clobber] "
	exit 0
elif [[ ! -f ${1} ]]; then
	echo "list does not exist...exiting"
	exit 1
# Checks that the gapping is set to one of the csstar presets
elif [[ "${2}" != "gapped" ]] && [[ "${2}" != "ungapped" ]]; then
...
if  [[ "${7}" == "MATRIX" ]] || [[ "${7}" == "SNV" ]] || [[ "${7}" == "BOTH" ]] || [[ "${7}" == "matrix" ]] || [[ "${7}" == "snv" ]] || [[ "${7}" == "both" ]]  [[ "${7}" == "Matrix" ]] || [[ "${7}" == "Snv" ]] || [[ "${7}" == "Both" ]]; then
	analysis_requested="${7^^}"
else
...
if [[ "${analysis_requested}" == "MATRIX" ]] || [[ "${analysis_requested}" == "BOTH" ]]; then
	...
	if [[ "${clobberness}" == "keep" ]]; then
		echo -e "\nMaking sure all isolates use the given AR Database - ${Alt_db}\n"
		while IFS= read -r line || [ -n "$line" ]; do
			...
			then
				#echo "${project}/${sample_name} has given database for normal csstar already"
				:
			else
			...
			if [[ -s "${OUTDATADIR}/GAMA/${sample_name}_${Alt_db}.GAMA" ]];	then
				mv "${OUTDATADIR}/GAMA/${sample_name}_${Alt_db}.GAMA" "${OUTDATADIR}/GAMA/${sample_name}.${Alt_db}.GAMA"
			fi
			if [[ -s "${OUTDATADIR}/GAMA/${sample_name}.${Alt_db}.GAMA" ]];	then
				#echo "${project}/${sample_name} has given database normal GAMA already"
				:
			else
				echo "${project}/${sample_name} - GAMA needs to be run against ${Alt_db}"
	...
	cp "${output_directory}/mashtree/${5}.dnd" "${output_directory}/${5}_mash.nwk"
	sed -i "s/_scaffolds_trimmed//g" "${output_directory}/${5}_mash.nwk"
	...
	if [[ "${run_csstar}" = "true" ]]; then
		...
		qsub -sync y ${shareScript}/abl_mass_qsub_alt_csstar.sh "${output_directory}/${5}-csstar_todo.txt" 25 "${8}" "${mass_qsub_folder}" "${clobberness}" "${sim}"
	fi
	...
	if [[ "${run_srst2}" = "true" ]]; then
		...
		qsub -sync y ${shareScript}/abl_mass_qsub_alt_srst2.sh "${output_directory}/${5}-srst2_todo.txt" 25 "${8}" "${mass_qsub_folder}" "${clobberness}"
	fi
	if [[ "${run_GAMA}" = "true" ]]; then
		echo "Submitting list for GAMA qsub analysis"
		qsub -sync y ${shareScript}/abl_mass_qsub_alt_GAMA.sh "${output_directory}/${5}-GAMA_todo.txt" 25 "${8}" "${mass_qsub_folder}" "${clobberness}"
	fi
	...
	while IFS= read -r line; do
		...
		GAMA_list=""
		# Extracts all AR genes from normal csstar output file and creates a lits of all genes that pass the filtering steps
	 	#echo "^^^^^^^^^^^^^^^^^^^ ${OUTDATADIR}/GAMA/${sample_name}.${Alt_db}.GAMA"
		if [[ -f "${OUTDATADIR}/GAMA/${sample_name}.${Alt_db}.GAMA" ]]; then
			GARDB_full="${OUTDATADIR}/GAMA/${sample_name}.${Alt_db}.GAMA"
			while IFS= read -r line; do
				# exit if no genes were found for the sample
				if [[ -z "${line}" ]]; then
					break
	...
	python3 "${shareScript}/project_parser.py" -c "${output_directory}/${5}-sample_summary_full.txt" -p "${output_directory}/${5}-plasmid_summary.txt" -o "${output_directory}/${5}_AR_plasmid_report.csv" -d "${Alt_db}"
