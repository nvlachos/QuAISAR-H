...
# Print all extracted info to primary file
	echo -e "${project}\t${sample_name}\t${taxonomy}\t${taxonomy_source_type}\t${confidence_info}\t${taxonomy_2}\t${taxonomy_source_type_2}\t${confidence_info_2}\t${mlst}\t${alleles}\t${alt_mlst}\t${alt_alleles}\t${csstar_list}\t${srst2_results}\t${GAMA_list}" >> ${output_directory}/${4}-sample_summary.txt
...
python3 "${shareScript}/project_parser_including_aniDB2.py" -s "${output_directory}/${4}-sample_summary.txt" -p "${output_directory}/${4}-plasmid_summary.txt" -o "${output_directory}/${4}_AR_plasmid_report_aniDB2.tsv" -d "${ResGANNCBI_srst2_filename}"
