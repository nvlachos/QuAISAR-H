...
make_relies_on_unzipped_fastqs() {
	...
	do
		...
		echo -e "bbduk.sh -\"${bbduk_mem}\" threads=\"${procs}\" in=\"${main_dir}/${sample}/FASTQs/${sample}_R1_001.fastq\" in2=\"${main_dir}/${sample}/FASTQs/${sample}_R2_001.fastq\" out=\"${main_dir}/${sample}/removedAdapters/${sample}-noPhiX-R1.fsq\" out2=\"${main_dir}/${sample}/removedAdapters/${sample}-noPhiX-R2.fsq\" ref=\"${shareScript}/phiX.fasta\" k=\"${bbduk_k}\" hdist=\"${bbduk_hdist}\"" >> "${main_dir}/BTQC_${sample}_${start_time}.sh"
