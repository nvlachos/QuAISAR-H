...
if [[ $# -eq 0 ]]; then
	echo "No argument supplied to run_kraken.sh, exiting"
	exit 1
elif [[ -z "${1}" ]]; then
...
if [ ! -d "$OUTDATADIR/kraken" ]; then
	...
	mkdir -p "$OUTDATADIR/kraken/${2}Assembly${full}"
elif [ ! -d "$OUTDATADIR/kraken/{2}Assembly${full}" ]; then 
	echo "Creating $OUTDATADIR/kraken/${2}Assembly${full}"
	mkdir -p "$OUTDATADIR/kraken/${2}Assembly${full}"
fi
...
kraken --db "${kraken_full_db}" --preload --threads "${procs}" --output "${OUTDATADIR}/kraken/full/${1}_full.kraken" --classified-out "${OUTDATADIR}/kraken/full/${1}_full.classified" "${OUTDATADIR}/Assembly/${1}_scaffolds_trimmed.fasta"
...
python ${shareScript}/Kraken_Assembly_Converter_2_Exe.py "${OUTDATADIR}/kraken/full/${1}_full.kraken"
kraken-translate --db "${kraken_full_db}" "${OUTDATADIR}/kraken/full/${1}_full_BP.kraken" > "${OUTDATADIR}/kraken/full/${1}_full_BP.labels"
kraken-mpa-report --db "${kraken_full_db}" "${OUTDATADIR}/kraken/full/${1}_full_BP.kraken" > "${OUTDATADIR}/kraken/full/${1}_full_weighted.mpa"
perl "${shareScript}/Methaplan_to_krona.pl" -p "${OUTDATADIR}/kraken/full/${1}_full_weighted.mpa" -k "${OUTDATADIR}/kraken/full/${1}_full_weighted.krona"
kraken-report --db "${kraken_full_db}" "${OUTDATADIR}/kraken/full/${1}_full_BP.kraken" > "${OUTDATADIR}/kraken/full/${1}_full_BP.list"
python ${shareScript}/Kraken_Assembly_Summary_Exe.py "${OUTDATADIR}/kraken/full/${1}_full_BP.kraken" "${OUTDATADIR}/kraken/full/${1}_full_BP.labels" "${OUTDATADIR}/kraken/full/${1}_full_BP.list" "${OUTDATADIR}/kraken/full/${1}_full_BP_data.list"
. "${shareScript}/module_changers/perl_5221_to_5123.sh"
ktImportText "${OUTDATADIR}/kraken/full/${1}_full_weighted.krona" -o "${OUTDATADIR}/kraken/full/${1}_full_weighted_BP_krona.html"
...
kraken-mpa-report --db "${kraken_full_db}" "${OUTDATADIR}/kraken/full/${1}_full.kraken" > "${OUTDATADIR}/kraken/full/${1}_full.mpa"

# Run the krona generator on the metaphlan output
echo "[:] Generating krona output for ${1}."
perl "${shareScript}/Methaplan_to_krona.pl" -p "${OUTDATADIR}/kraken/full/${1}_full.mpa" -k "${OUTDATADIR}/kraken/full/${1}_full.krona"

# Run the krona graph generator from krona output
. "${shareScript}/module_changers/perl_5221_to_5123.sh"
ktImportText "${OUTDATADIR}/kraken/full/${1}_full.krona" -o "${OUTDATADIR}/kraken/full/${1}_full.html" 
. "${shareScript}/module_changers/perl_5123_to_5221.sh"

# Creates the parsible report from the kraken output
echo "[:] Creating alternate report for taxonomic extraction"
kraken-report --db "${kraken_full_db}" "${OUTDATADIR}/kraken/full/${1}_full.kraken" > "${OUTDATADIR}/kraken/full/${1}_full.list"
